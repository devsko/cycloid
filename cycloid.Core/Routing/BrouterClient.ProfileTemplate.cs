...
partial class BrouterClient
{
    private static readonly string _profileTemplate = """
        #
        ...
        assign downhillcutoff     = {downhillcutoff}   # %downhillcutoff% | Gradients below this value in percents are not counted. | number
        assign uphillcost         = {uphillcost}     # %uphillcost% | Cost for going uphill | number
        assign uphillcutoff       = {uphillcutoff}   # %uphillcutoff% | Gradients below this value in percents are not counted.  | number
        ...
        assign totalMass  = 105     # %totalMass% | Mass (in kg) of the bike + biker, for travel time computation | number
        assign maxSpeed   = 65     # %maxSpeed% | Absolute maximum speed (in km/h), for travel time computation | number
        assign S_C_x      = 0.15  # %S_C_x% | Drag coefficient times the reference area (in m^2), for travel time computation | number
        assign C_r        = 0.01   # %C_r% | Rolling resistance coefficient (dimensionless), for travel time computation | number
        assign bikerPower = {bikerPower}    # %bikerPower% | Average power (in W) provided by the biker, for travel time computation | number
        ...
        assign isgoodgravel and surface=compacted or any_cycleroute or smoothness=intermediate tracktype=grade2 
        assign isunpaved not or surface= or ispaved or isgoodgravel or surface=fine_gravel surface=cobblestone
        ...
        assign costfactor
          ...
          switch    highway=cycleway                          1.15
          switch    or isresidentialorliving highway=service                     
           switch isunpaved 10 
           switch isgoodgravel 1.4 1.2
          switch or highway=track or highway=road or highway=path highway=footway
           ...
           switch tracktype=grade2 
            switch isunpaved 10
            switch or ispaved isgoodgravel 1.25 3
